...
class SampleDecoderOnlyOutput(ModelOutput):
    ...
    presafe_layer_dist: Optional[Dict[int, int]] = None
...
class NGramChecker:
    def __init__(self):
        self.generated_ngrams = set()
    
    def check_no_repeat_ngram(self, n, generated_ids):
        # 获取最后一个生成的序列
        last_sequence = generated_ids[-1]

        # 取出最后一个序列中的最后 n 个元素
        ngram = tuple(last_sequence[-n:])

        
        if len(ngram) < n:
            return False
        
        if ngram in self.generated_ngrams:
            return True
        else:
            self.generated_ngrams.add(ngram)
            return False
def log_to_json(toxic_layer, presafe_max_layer,base_layer,filepath):
    """
    将 toky_layer 和 presafe_max_layer 记录到 JSON 文件中。
    :param toxic_layer: 有毒层的值
    :param presafe_max_layer: 安全层的最大值
    ...
    try:
        ...
        log_data = {
            ...
            "presafe_max_layer": presafe_max_layer,
            "base_layer":base_layer,
        }
        
        # 检查文件是否存在，如果存在则追加，否则创建文件
...
def close_json_log(filepath):
    """
    关闭JSON文件的数组（如果文件存在的话）。
    :param filepath: 日志文件的路径
    """
    try:
        if os.path.exists(filepath):
            with open(filepath, 'a') as f:
                f.write(']\n')  # 结束文件时补充关闭 JSON 数组的括号
    except Exception as e:
...
class GenerationMixin:
    ...
    def generate(
        ...
        dscd_decoding: Optional[bool] = None,
        mature_layer: Optional[int] = None,
        toxic_layer: Optional[int] = None,
        safe_layer: Optional[int] = None,
        base_layer: Optional[int] = None,
        no_repeat_ngram_size: Optional[int] = None,
        candidate_premature_layers: Optional[List[int]] = None,
        relative_top: Optional[float] = 0.1,
        alpha: Optional[float] = 1.0,  # 成熟度对比权重
        beta: Optional[float] = 1.0,   # 安全性对比权重
        debug_dscd: Optional[bool] = False,  # 是否输出调试信息
        ...
        **kwargs,
        ...
        elif is_sample_gen_mode and dscd_decoding:
    ...
    def dscd_sample(
        ...
        **model_kwargs,
        ...
        with torch.no_grad():
            while True:
                ...
                if(candidate_premature_layers is not None):
                    ...
                    presafe_layer_dist = {l:0 for l in early_exit_layers}
                ...
                if base_layer is not None:
                    ...
                    # 1. 获取所有层的隐藏状态
                    all_hidden_states = torch.stack([dict_outputs[i][:, -1, :] for i in range(len(dict_outputs))], dim=0)

                    # 2. 获取除最后一层之外的所有层的隐藏状态
                    stacked_premature_layers = all_hidden_states[:-1, :, :]  # 选择除最后一层之外的所有层

                    # 3. 获取早期退出层的隐藏状态
                    stacked_presafe_layers = all_hidden_states[early_exit_layers, :, :]  # 选择早期退出层
                    ...
                    softmax_toxic_max_layer = F.softmax(dict_outputs[toxic_layer[0]][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
                    # softmax_toxic_layer = F.softmax(dict_outputs[toxic_layer[0]][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
                    softmax_toxic_other_layer = F.softmax(dict_outputs[toxic_layer[1]][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
                    softmax_presafe_layers = F.softmax(stacked_presafe_layers, dim=-1)  # shape: (num_premature_layers, batch_size, num_features)

                    # 3. Calculate M, the average distribution
                    M = 0.5 * (softmax_mature_layer[None, :, :] + softmax_premature_layers)  # shape: (num_premature_layers, batch_size, num_features)

                    # N = 0.5 * (softmax_toxic_layer[None, :, :] + softmax_presafe_layers)  # shape: (num_premature_layers, batch_size, num_features)
                    N_1 = 0.5 * (softmax_toxic_max_layer[None, :, :] + softmax_presafe_layers)  # shape: (num_premature_layers, batch_size, num_features)
                    N_2 = 0.5 * (softmax_toxic_other_layer[None, :, :] + softmax_presafe_layers)  # shape: (num_premature_layers, batch_size, num_features)

                    # 4. Calculate log-softmax for the KL divergence
                    log_softmax_mature_layer = torch.log(softmax_mature_layer)
                    ...
                    log_softmax_toxic_max_layer = torch.log(softmax_toxic_max_layer)
                    log_softmax_toxic_other_layer = torch.log(softmax_toxic_other_layer)
                    log_softmax_presafe_layers = F.log_softmax(stacked_presafe_layers, dim=-1)  # shape: (num_premature_layers, batch_size, num_features)
                    ...
                    kl3_1 = F.kl_div(log_softmax_toxic_max_layer[None, :, :], N_1, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
                    kl3_2 = F.kl_div(log_softmax_toxic_other_layer[None, :, :], N_2, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
                    kl4_1 = F.kl_div(log_softmax_presafe_layers, N_1, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
                    kl4_2 = F.kl_div(log_softmax_presafe_layers, N_2, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
                    js_safe_max_divs = 0.5 * (kl3_1 + kl4_1)  # shape: (num_premature_layers, batch_size)
                    js_safe_other_divs = 0.5 * (kl3_2 + kl4_2)  # shape: (num_premature_layers, batch_size)
                    # 6. Reduce the batchmean
                    js_divs = js_divs.mean(-1)  # shape: (num_premature_layers,)
                    premature_layer = candidate_premature_layers[int(js_divs.argmax().cpu().item())]
                    premature_layer_dist[premature_layer] += 1

                    js_divs_safe_max_mean = js_safe_max_divs.mean(-1)  # shape: (num_premature_layers,)
                    js_divs_safe_other_mean = js_safe_other_divs.mean(-1)  # shape: (num_premature_layers,)
                    js_divs_safe_max_mean[0] = float('-inf')
                    js_divs_safe_other_mean[0] = float('-inf')
                    top_two_indices_max = js_divs_safe_max_mean.topk(2).indices.cpu().tolist()
                    top_two_indices_other = js_divs_safe_other_mean.topk(2).indices.cpu().tolist()
                    presafe_max_layer = early_exit_layers[top_two_indices_max[0]]
                    presafe_other_layer = early_exit_layers[top_two_indices_other[0]]
                    # print("safe_layer:{}".format(presafe_layer))
                    # print("safe_layer:{}".format(safe_layer))
                    # safe_layer_dist[presafe_layer] += 1
                    # presafe_layer = candidate_premature_layers[int(js_safe_divs.argmax().cpu().item())]
                    # presafe_layer_dist[presafe_layer] += 1


                    # safe_logits = dict_outputs[presafe_layer][:, -1, :]
                    safe_max_logits = dict_outputs[presafe_max_layer][:, -1, :]
                    safe_other_logits = dict_outputs[presafe_other_layer][:, -1, :]
                    # toxic_logits = dict_outputs[toxic_layer][:, -1, :]
                    toxic_max_logits = dict_outputs[toxic_layer[0]][:, -1, :]
                    toxic_other_logits = dict_outputs[toxic_layer[1]][:, -1, :]


                    base_logits = dict_outputs[premature_layer][:, -1, :]
                    final_logits = dict_outputs[mature_layer][:, -1, :]
                    # if relative_top > 0.0:
                    #     # safe_logits = self.relative_top_filter(safe_logits, relative_top)
                    #     toxic_logits = toxic_logits.log_softmax(dim=-1)
                    #     # safe_logits = safe_logits.log_softmax(dim=-1)
                    #     # toxic_logits = toxic_logits.log_softmax(dim=-1)

                    #     # mask_s = safe_logits[0] < -1e3
                    #     # toxic_logits[0][mask_s] = -1e3

                    #     # final_logits = self.relative_top_filter(final_logits, relative_top)
                    #     # safe_logits = self.relative_top_filter_safe(safe_logits, relative_top)
                    #     # if(toxic_layer<=13):
                    #     final_logits = self.relative_top_filter(final_logits, relative_top)
                    #     base_logits = base_logits.log_softmax(dim=-1)
                    ...
                    #     mask = final_logits[0] < -1e3
                    #     base_logits[0][mask] = -1e3
                    #     toxic_logits[0][mask] = -1e3
                    #     bad_logits[0][mask]=-1e3
                    #     safe_logits[0][mask]=-1e3
                    if relative_top > 0.0:
                        #get_relative_top_filter函数可能用于创建一个mask，该mask选择top-k概率最高的tokens。
                        # print("diff_logits_origin："+str(diff_logits))#调试，diff_logits处理前
                        relative_top_mask = self.get_relative_top_filter(final_logits, relative_top)#
                        # relative_top_mask_safe = self.get_relative_top_filter(safe_logits, relative_top)#
                        # 获取前 10% 的索引

                        # diff_logits = good_logits-bad_logits
                        bad_logits = toxic_max_logits+toxic_other_logits-safe_other_logits-safe_max_logits
                        bst_logits = base_logits-safe_max_logits
                        final_logits = torch.where(relative_top_mask, -float("Inf"), final_logits)
                        bad_logits = torch.where(relative_top_mask, -1e3, bad_logits)
                        bst_logits = torch.where(relative_top_mask, -1e3, bst_logits)
                        base_logits = torch.where(relative_top_mask, -1e3, base_logits)
                        # toxic_logits = torch.where(relative_top_mask, -1e3, toxic_logits)
                        # safe_logits = torch.where(relative_top_mask, -1e3, safe_logits)
                        ...
                        bad_logits = bad_logits.log_softmax(dim=-1)
                        bst_logits = bst_logits.log_softmax(dim=-1)
                        base_logits = base_logits.log_softmax(dim=-1)
                        # toxic_logits = toxic_logits.log_softmax(dim=-1)


                    # logits = final_logits - base_logits-toxic_logits
                    # print("bad_logits:{}", bad_logits)
        ...
        #         # print("presafe_layer_dist:{}", presafe_layer_dist)
        #         # print("premature_layer_dist:{}", premature_layer_dist)
        ...
        #             if checker.check_no_repeat_ngram(no_repeat_ngram_size, generated_ids):
        #                 # 使用 -inf 屏蔽不允许的 token
        #                 specific_token_index = -1  # 这里根据实际情况设定索引
        #                 next_token_scores[:, specific_token_index] = -float("Inf")


        #         # 打印结果
        #         # print("不是 -inf 的值的位置:", not_inf_indices)
        #         # print("对应的值:", not_inf_values)
        #         # Store scores, attentions and hidden_states when required
        #         if return_dict_in_generate:
        #             if output_scores:
        ...
        #         # sample
        #         probs = nn.functional.softmax(next_token_scores.float(), dim=-1)  # Ensure softmax is float32
        #         probs = torch.clamp(probs, min=1e-10)

        #         # Check for any anomalies
        #         if torch.any(torch.isinf(probs)) or torch.any(torch.isnan(probs)) or torch.any(probs < 0):
        #             raise ValueError("probs contain inf, nan, or negative values after clamping")

        #         # Ensure probabilities sum to 1
        #         if not torch.allclose(probs.sum(dim=-1), torch.tensor(1.0)):
            ...
            if return_dict_in_generate:
                ...
                if self.config.is_encoder_decoder:
                    ...
                    return SampleDecoderOnlyOutput(
                        ...
                        presafe_layer_dist=presafe_layer_dist,
    ...
    def greedy_search(
        ...
        candidate_premature_layers: Optional[List[int]] = None,
        relative_top: float = 0.1,
        alpha: float = 1.0,  # 成熟度对比权重
        beta: float = 1.0,   # 安全性对比权重
        debug_dscd: bool = True,  # 是否输出调试信息
        logits_processor: Optional[LogitsProcessorList] = None,
        stopping_criteria: Optional[StoppingCriteriaList] = None,
        max_length: Optional[int] = None,
        pad_token_id: Optional[int] = None,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # 1. Stacking all premature_layers into a new dimension
                ...
                # 3. 获取早期退出层的隐藏状态
                # stacked_presafe_layers = all_hidden_states[early_exit_layers, :, :]  # 选择早期退出层
                stacked_presafe_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_premature_layers], dim=0)
                #只有大于16的才有作为安全层的资格，反之没有资格
                # stacked_presafe_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_premature_layers if i > 16], dim=0)
                ...
                # js_safe_max_divs = 0.5 * (klmix + kl4_1) 
                # js_divs_safe = js_divs_safe.mean(-1)
                js_safe_max_divs = 0.5 * (kl3_1 + kl4_1)  # OLD
                ...
                top_two_indices_max = js_divs_safe_max_mean.topk(2).indices.cpu().tolist()
                # top_two_indices_other = js_divs_safe_other_mean.topk(2).indices.cpu().tolist()
                presafe_max_layer = early_exit_layers[top_two_indices_max[0]]###new
                # presafe_max_layer = candidate_premature_layers[int(js_divs_safe.argmax().cpu().item())]
                ...
                base_logits = dict_outputs[premature_layer][:, -1, :]
                final_logits = dict_outputs[mature_layer][:, -1, :]
                # log_to_json(toxic_layer, presafe_max_layer,premature_layer,"/mnt/sdb/zjk/EasyEdit_dscd/LLAMA/DDSC+DINMvsDINM_ORIPROMPT/layer.json")

                # 建议3：调试输出
                if debug_dscd:
                    print(f"\n[DSCD Debug] Token position: {input_ids.shape[-1]}")
                    print(f"  Mature layer: {mature_layer}, Premature layer: {premature_layer}")
                    print(f"  Safe layer: {presafe_max_layer}, Toxic layer: {toxic_layer[0]}")
                    print(f"  Alpha (maturity weight): {alpha}, Beta (safety weight): {beta}")

                # 方案 D：完全模仿 DoLA 的实现
                # 关键：final_logits 保持在原始 logits 空间，bst_logits 在对数概率空间
                
                # 步骤 1：创建组合基线（在原始 logits 空间）
                # bst_logits = base - beta*safe + beta*toxic
                bst_logits = beta * toxic_max_logits + base_logits - beta * safe_max_logits
                
                # 步骤 2：对 final_logits 应用 relative_top filter（保持在原始空间）
                if relative_top > 0.0:
                    # 获取 mask
                    relative_top_mask = self.get_relative_top_filter(final_logits, relative_top)
                    
                    # 对 final_logits 应用 mask（保持在原始 logits 空间）
                    # 使用 -1e3 而不是 -1e10，模仿 DoLA
                    final_logits = torch.where(relative_top_mask, -1e3, final_logits)
                    
                    # 对 bst_logits 进行 log_softmax（转到对数概率空间）
                    bst_logits = bst_logits.log_softmax(dim=-1)
                    
                    # 对 bst_logits 应用相同的 mask
                    bst_logits = torch.where(relative_top_mask, -1e3, bst_logits)
                
                # 步骤 3：DoLA 风格的不对称对比
                # final_logits: 原始 logits 空间
                # bst_logits: 对数概率空间
                # 这种"不对称"是 DoLA 的关键技巧
                logits = alpha * (final_logits - bst_logits)
                
                # 建议3：调试输出 - 显示对比项的统计信息
                if debug_dscd:
                    contrast = final_logits - bst_logits
                    print(f"  Contrast (final - bst) - mean: {contrast.mean().item():.4f}, std: {contrast.std().item():.4f}")
                    print(f"  Final logits range: [{final_logits.min().item():.2f}, {final_logits.max().item():.2f}]")
                    print(f"  BST logits range: [{bst_logits.min().item():.2f}, {bst_logits.max().item():.2f}]")
                    # 显示 top-5 tokens
                    top5_values, top5_indices = logits.topk(5)
                    print(f"  Top-5 token IDs: {top5_indices[0].tolist()}")
                    print(f"  Top-5 logits values: {top5_values[0].tolist()}")
            ...
            #     top_two_indices_max = js_divs_safe_max_mean.topk(2).indices.cpu().tolist()
            #     # top_two_indices_other = js_divs_safe_other_mean.topk(2).indices.cpu().tolist()
            #     presafe_max_layer = early_exit_layers[top_two_indices_max[0]]###old
            #     # presafe_max_layer = candidate_premature_layers[int(js_divs_safe.argmax().cpu().item())]

            #     # safe_logits = dict_outputs[presafe_layer][:, -1, :]
            #     safe_max_logits = dict_outputs[safe_layer][:, -1, :]
            #     # safe_other_logits = dict_outputs[presafe_other_layer][:, -1, :]
            #     # toxic_logits = dict_outputs[toxic_layer][:, -1, :]
            #     toxic_max_logits = dict_outputs[toxic_layer[0]][:, -1, :]
            #     # toxic_other_logits = dict_outputs[toxic_layer[1]][:, -1, :]
            ...
            #         # diff_logits = good_logits-bad_logits
            #         # bad_logits = toxic_max_logits+toxic_other_logits-safe_other_logits-safe_max_logits
            #         bst_logits = base_logits-safe_max_logits+toxic_max_logits
            #         final_logits = torch.where(relative_top_mask, -float("Inf"), final_logits)
            #         # bad_logits = torch.where(relative_top_mask, -1e3, bad_logits)
            #         bst_logits = torch.where(relative_top_mask, -1e3, bst_logits)
            #         # base_logits = torch.where(relative_top_mask, -1e3, base_logits)
            ...
            #     # logits = final_logits - base_logits
            #     # if dscd==True:
            #     logits = final_logits-bst_logits
            #     # else:
            #     #     logits = final_logits - base_logits
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                ...
                return GreedySearchEncoderDecoderOutput(
                    ...
                    cross_attentions=cross_attentions,
                    decoder_hidden_states=decoder_hidden_states,
                ),presafe_max_layer,toxic,premature_layer
            else:
            ...
            return input_ids,presafe_max_layer,toxic,premature_layer
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        #         output_hidden_states=output_hidden_states,
        #     )
        #     next_token_logits_1 = outputs.logits[:, -1, :]
        #     # print(next_token_logits_1.shape)
        ...
        #     # # 1. Stacking all premature_layers into a new dimension
        #     # stacked_premature_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_premature_layers], dim=0)
        #     # stacked_presafe_layers = torch.stack([dict_outputs[i][:, -1, :] for i in early_exit_layers], dim=0)#将候选过早层所有的隐藏状态叠加到第0维

        #     # # 2. Calculate the softmax values for mature_layer and all premature_layers
        #     # softmax_mature_layer = F.softmax(dict_outputs[mature_layer][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
        #     # softmax_premature_layers = F.softmax(stacked_premature_layers, dim=-1)  # shape: (num_premature_layers, batch_size, num_features)
        #     # softmax_toxic_layer = F.softmax(dict_outputs[toxic_layer][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
        #     # softmax_presafe_layers = F.softmax(stacked_presafe_layers, dim=-1)  # shape: (num_premature_layers, batch_size, num_features)
        ...
        #     # log_softmax_toxic_layer = F.log_softmax(dict_outputs[toxic_layer][:, -1, :], dim=-1)  # shape: (batch_size, num_features)
        #     # log_softmax_presafe_layers = F.log_softmax(stacked_presafe_layers, dim=-1)  # shape: (num_premature_layers, batch_size, num_features)
        #     # # 5. Calculate the KL divergences and then the JS divergences
        #     # kl1 = F.kl_div(log_softmax_mature_layer[None, :, :], M, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
        #     # kl2 = F.kl_div(log_softmax_premature_layers, M, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
        ...
        #     # kl3 = F.kl_div(log_softmax_toxic_layer[None, :, :], N, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
        #     # kl4 = F.kl_div(log_softmax_presafe_layers, N, reduction='none').mean(-1)  # shape: (num_premature_layers, batch_size)
        #     # js_safe_divs = 0.5 * (kl3 + kl4)  # shape: (num_premature_layers, batch_size)
        #     # # 6. Reduce the batchmean
        #     # js_divs = js_divs.mean(-1)  # shape: (num_premature_layers,)
        #     # premature_layer = candidate_premature_layers[int(js_divs.argmax().cpu().item())]
        #     # premature_layer_dist[premature_layer] += 1

        #     # js_divs_safe_mean = js_safe_divs.mean(-1)  # shape: (num_premature_layers,)
        #     # js_divs_safe_mean[0] = float('-inf')
        #     # top_two_indices = js_divs_safe_mean.topk(2).indices.cpu().tolist()
        #     # presafe_layer = early_exit_layers[top_two_indices[0]]
        #     # # print("safe_layer:{}".format(presafe_layer))
        #     # # print("safe_layer:{}".format(safe_layer))
        #     # # safe_layer_dist[presafe_layer] += 1
        #     # # presafe_layer = candidate_premature_layers[int(js_safe_divs.argmax().cpu().item())]
        ...
        #     # # logits = final_logits - base_logits
        #     # # logits = final_logits - bad_logits
        #     # logits = final_logits
        #     # print(logits.shape)
        ...
        #     # pre-process distribution
        #     next_token_scores = logits_processor(input_ids, next_token_logits_1)
        #     next_token_scores = logits_warper(input_ids, next_token_scores)
        #     # if no_repeat_ngram_size > 0:
        #     #     if checker.check_no_repeat_ngram(no_repeat_ngram_size, generated_ids):
        #     #         # 使用 -inf 屏蔽不允许的 token
