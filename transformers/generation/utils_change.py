...
class GenerationMixin:
    ...
    # def relative_top_filter(self, scores: torch.FloatTensor, relative_top: float = 0.1, min_tokens_to_keep: int = 1):
    #     scores_normalized = scores.log_softmax(dim=-1) 
    #     sorted_logits, sorted_indices = torch.sort(scores_normalized, descending=True)
    #     min_thresh = sorted_logits[..., min_tokens_to_keep-1] 
    #     probs_max = torch.max(scores_normalized, dim=-1).values
    ...
    # def relative_top_filter_safe(self, scores: torch.FloatTensor, relative_top: float = 0.1, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1) -> torch.FloatTensor:
    #     assert 0 < relative_top <= 1, "relative_top 必须在 0 和 1 之间"

    #     # 检查输入分数是否有无效值
    #     if torch.isnan(scores).any() or torch.isinf(scores).any():
    #         raise ValueError("输入的 scores 张量包含 NaN 或 Inf 值")

    #     scores_normalized = scores.log_softmax(dim=-1)
        
    #     # 检查 log_softmax 后的分数是否有无效值
    #     if torch.isnan(scores_normalized).any() or torch.isinf(scores_normalized).any():
    #         raise ValueError("log_softmax 后的 scores_normalized 张量包含 NaN 或 Inf 值")
    ...
    #     # 检查计算后的阈值是否有无效值
    #     if torch.isnan(probs_thresh).any() or torch.isinf(probs_thresh).any():
    #         raise ValueError("计算后的 probs_thresh 张量包含 NaN 或 Inf 值")
    ...
    #     # 额外的归一化步骤以确保有效的概率分布
    #     scores_filtered = torch.nn.functional.softmax(scores_normalized, dim=-1)
        
    #     # 检查最后的输出是否有无效值
    #     if torch.isnan(scores_filtered).any() or torch.isinf(scores_filtered).any():
    #         raise ValueError("过滤后的 scores_filtered 张量包含 NaN 或 Inf 值")
...
###加入safe_logits
    def dola_greedy_decode(
        ...
        **model_kwargs,
        ...
        if base_layer is not None and candidate_premature_layers is None:
            early_exit_layers = [base_layer, mature_layer,toxic_layer]
            num_base_layers = 1
            premature_layer_dist = {}
            presafe_layer_dist = {}
        elif candidate_premature_layers is not None:
            early_exit_layers = candidate_premature_layers + [mature_layer]+[toxic_layer]
            num_base_layers = len(candidate_premature_layers)
            premature_layer_dist = {l:0 for l in candidate_premature_layers}
            presafe_layer_dist = {l:0 for l in candidate_premature_layers}
        else:
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                stacked_presafe_layers = torch.stack([dict_outputs[i][:, -1, :] for i in candidate_premature_layers], dim=0)#将候选过早层所有的隐藏状态叠加到第0维
                ...
                js_safe_divs = js_safe_divs.mean(-1)  # shape: (num_premature_layers,)
                ...
                final_logits = dict_outputs[mature_layer][:, -1, :]
                # add_logits = final_logits+safe_logits
                if relative_top > 0.0:
                ...
                bad_logits = toxic_logits+base_logits
                # logits = final_logits - base_logits+safe_logits-toxic_logits
                if(toxic_layer<=13):
                    logits = final_logits - base_logits
                else:
                    logits = good_logits - bad_logits
    ...
    def contrastive_greedy_decode(
        ...
        **model_kwargs,
        ...
        student_model_kwargs_safe = copy.deepcopy(model_kwargs)
        student_model_kwargs_toxic = copy.deepcopy(model_kwargs)
        while True:
            if synced_gpus:
                # Under synced_gpus the `forward` call must continue until all gpus complete their sequence.
                # The following logic allows an early break if all peers finished generating their sequence
            ...
            model_inputs_safe = self.prepare_inputs_for_generation(input_ids, **student_model_kwargs_safe)
            student_model_inputs_toxic = self.prepare_inputs_for_generation(input_ids, **student_model_kwargs_toxic)
            ...
            student_outputs_safe = student_model(
                **model_inputs_safe,
                return_dict=True,
                output_attentions=output_attentions,
                output_hidden_states=output_hidden_states,
            )
            # student_outputs_toxic = student_model(
            #     **model_inputs_toxic,
            #     return_dict=True,
            #     output_attentions=output_attentions,
            #     output_hidden_states=output_hidden_states,
            ...
            if synced_gpus and this_peer_finished:
                continue  # don't waste resources running the code we don't need
            # safe_logits = student_outputs_safe[0][:, -1, :]
            # toxic_logits = student_outputs_toxic[0][:, -1, :]
    ...
    def dola_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            next_token_scores = logits_warper(input_ids, next_token_scores)
            # 检查哪些值不是 -inf
            not_inf_mask = ~torch.isinf(next_token_scores)

            # 获取不是 -inf 的值的位置
            not_inf_indices = torch.nonzero(not_inf_mask, as_tuple=False)

            # 获取对应的位置和值
            not_inf_values = next_token_scores[not_inf_mask]
